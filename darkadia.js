...
const Emojis = require("./src/darkadia/emojis.json")
const Colors = require("./src/darkadia/colors.json")
const Logs = require("./src/darkadia/logs.json")
...
process.on("uncaughtException", hata => { 
  const Hata2 = new EmbedBuilder()
    ...
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  client.channels.cache.get("1217072142542180443").send({embeds: [Hata2]}) 
})
process.on("uncaughtExceptionMonitor", hata => {
  const Hata3 = new EmbedBuilder()
    ...
    .setDescription(`> Bot bir hata verdi.\n\n> \`${hata}\``) 
    .setTimestamp()
  client.channels.cache.get("1217072142542180443").send({embeds: [Hata3]}) 
}) 
//=====// Hata log \\=====\\
