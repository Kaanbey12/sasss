...
async execute(client, interaction) { 
  ...
  const Sahip = new EmbedBuilder()
    ...
    .setTimestamp()
  if(interaction.user.id !== "1031841616023007282" && interaction.user.id !== "1031841616023007282" && interaction.user.id !== "1031841616023007282") return await interaction.followUp({embeds: [Sahip]})
  ...
  if(interaction.options.getSubcommand() === 'ekle') {
    ...
    const Eklendi = new EmbedBuilder()
      ...
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Tik} ${Kullanıcı} adlı kullanıcının bakiyesine **${Miktar} AD** eklendi.`)
       .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
  ...
  if(interaction.options.getSubcommand() === 'al') {
    ...
    const ParaYok = new EmbedBuilder()
      .setColor(Colors.Red)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Çarpı} Bu kullanıcının bakiyesinde **${Miktar} AD** bulunmuyor.`)
      .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
    ...
    const Alındı = new EmbedBuilder()
      .setColor(Colors.Green)
      .setAuthor({name: interaction.user.username, iconURL: interaction.user.avatarURL()}) 
      .setDescription(`${Emojis.Tik} ${Kullanıcı} adlı kullanıcının bakiyesinden **${Miktar} AD** alındı.`)
       .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
      .setTimestamp()
