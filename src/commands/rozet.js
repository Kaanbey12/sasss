...
module.exports = {
  ...
  dbl: false, 
  name: ['rozet'],  
  description: 'DarkAdia rozet sistemi.',
  options: [
    { 
      name: "ver", 
...
async execute(client, interaction) {
  ...
  if(interaction.options.getSubcommand() === 'liste') {
    ...
    const Liste = new EmbedBuilder()
      ...
      .setDescription(`
> ${Emojis.RozetKullanıcı} DarkAdia botunu kullanan bütün kullanıcılarda bulunmaktadır.

> ${Emojis.RozetSponsor} DarkAdia botuna sponsor olan kullanıcılara verilir.

> ${Emojis.RozetDestekçi} DarkAdia botuna veya sunucusuna destekde bulunan kullanıcılara verilir.

> ${Emojis.RozetPremium} DarkAdia premium sahibi üyelere verilir.

> ${Emojis.RozetBug} DarkAdia botunda önemli bir bug bulan kullanıcılara verilir.

> ${Emojis.RozetKurucu} DarkAdia kurucularına verilir.

> ${Emojis.RozetDeveloper} DarkAdia geliştiricilerine verilir.
...
`)
  ...
  if(interaction.options.getSubcommand() === 'ver') {
   ...
   const Sahip = new EmbedBuilder()
    ...
    .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
    .setTimestamp()
  if(interaction.user.id !== "1031841616023007282" && interaction.user.id !== "1031841616023007282" && interaction.user.id !== "1031841616023007282" && interaction.user.id !== "1031841616023007282") return await interaction.followUp({embeds: [Sahip]})
