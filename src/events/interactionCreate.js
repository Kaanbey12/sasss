      ...
      try {
        ...
        if(db.fetch(`Karaliste_${interaction.user.id}`)) {
          const Kullanamazsın = new EmbedBuilder()
            ...
            .setTimestamp()
          if(interaction.user.id !== "1167500081281368146" && interaction.user.id !== "1031841616023007282" && interaction.user.id !== "") return await interaction.reply({embeds: [Kullanamazsın], components: [Buton]})
        }
        ...
        if(db.fetch(`Bakım`)) {
          const Bakımda = new EmbedBuilder()
            ...
            .setFooter({text: client.user.username, iconURL: client.user.avatarURL()}) 
            .setTimestamp()
          if(interaction.user.id !== "" && interaction.user.id !== "1031841616023007282" && interaction.user.id !== "1167500081281368146") return await interaction.reply({embeds: [Bakımda], components: [Buton]})
        }
